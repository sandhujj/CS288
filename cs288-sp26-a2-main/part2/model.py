...
def scaled_dot_product_attention(
    ...
    if mask is not None:
        ...
        normalizer = attn.sum(dim=-1, keepdim=True)
        attn = torch.where(normalizer > 0, attn / normalizer, torch.zeros_like(attn))
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        mask = self._create_causal_mask(seq_len, x.device)[None, None, :, :]
...
def count_flops_per_token(
    ...
    attn_scores = num_heads * (2 * context_length * d_k)
    attn_values = num_heads * (2 * context_length * d_k)
    ffn = 2 * d_model * d_ff + 2 * d_model * d_ff + 2 * d_ff * d_model
    layer_norm = 2 * (4 * d_model)
    output_proj = 2 * d_model * vocab_size
    return int(num_layers * (attn_proj + attn_scores + attn_values + ffn + layer_norm) + output_proj)
