...
class Tokenizer:
    ...
    def _encode_chunk(self, text: str) -> list[int]:
        ...
        for match in self.pat.finditer(text):
            bpe_tokens = self._bpe(match.group(0).encode("utf-8"))
            for bpe_token in bpe_tokens:
                token_id = inv_vocab.get(bpe_token)
                if token_id is not None:
                    ids.append(token_id)
                else:
                    ids.extend(inv_vocab[bytes([b])] for b in bpe_token)
