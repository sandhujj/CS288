...
def train_bpe(
    ...
    pretoken_freqs = Counter(pre_tokenize(text, special_tokens))
    word_freqs = Counter()
    word_cache = {}
    for pretoken, freq in pretoken_freqs.items():
        token_bytes = pretoken.encode("utf-8")
        if forbidden_substrings and any(sub in token_bytes for sub in forbidden_substrings):
            continue
        word = word_cache.get(token_bytes)
        if word is None:
            word = tuple(bytes([b]) for b in token_bytes)
            word_cache[token_bytes] = word
        if word:
    ...
    while len(vocab) < vocab_size and pair_counts:
        ...
        while pair_counts and (pair_counts[best_pair] <= 0 or not affected_words):
